...
## ★ THRESHOLD 갱신 (2026-07-01 직독 검증 — 별표 자산 확인)

**전제 정정:** 이 문서(6/22)는 THRESHOLD를 "별표 미수집 → Post-MVP"로 두었으나, 현재 DB 직독 결과 **별표는 법제처 API로 이미 수집·구조화 완료**되어 있음. THRESHOLD 병목은 "수집"이 아니라 "운영 연결(활성화)"임.

### 직독 확정 (Supabase `vwlahtguyggrhvslabax`, 서울)
| 데이터 | 위치 | 상태 |
|---|---|---|
| 별표 원문 (법제처 API 수집) | `appendix_runtime_metadata` **31,974건** | ✅ 수집 완료 (`raw_text` + `structured_json`) |
| 별표 THRESHOLD 추출 | `appendix_runtime_metadata` where metadata_type='THRESHOLD' **24,626건** | ✅ 구조화 완료 (operator/value/unit) |
| metadata_type 분포 | THRESHOLD 24,626 / WHO 5,786 / SCHEDULE 1,547 / CONDITION 6 / TARGET 4 / HOW 4 / WHEN 1 | — |
| 운영 발화 (obligation_instance) | trigger_type=THRESHOLD **3건** (worker_count만) | ❌ 미연결 |

- `structured_json` 예: `{"unit":"C","value":"1","operator":"이내"}`, `{"type":"distance","source":"별표8"}`, `{"type":"quantity","source":"별표9"}` — 이미 임계가 구조화됨.
- 별표 원천은 `산업안전보건기준에 관한 규칙 별표 8(안전거리)`, `별표 9(계측장치 기준량)` 등 산안 별표 포함.

### 병목 재정의
```
별표 수집(법제처 API)       ✅ appendix_runtime_metadata 31,974
별표 THRESHOLD 구조화        ✅ 24,626 (structured_json)
        ↓ (여기가 병목: 미연결)
운영 obligation_instance     ❌ 3건 (worker_count만 발화)
```
→ THRESHOLD 고도화 = **별표 THRESHOLD 24,626 자산을 Applicability Engine(의무 생성)이 읽어 obligation_instance로 발화시키는 연결(활성화)**. 신규 수집·신규 매핑 아님.
→ 6/25 헌법 "THRESHOLD 별표 임계값 확장 = 품질 개발 ① Applicability 내부"와 동일 지점.
→ Route B(applicability_conditions 14건 경유)는 소량 파일럿이었고, 실제 대량 임계 자산은 `appendix_runtime_metadata`에 있음.

### 오정정 기록 (혼동 방지)
- `law_attachment`(1,322건, 텍스트 1,306)는 **전기·기술기준 PDF 첨부**로, 산안 임계표가 아님("상시근로자" 0건). THRESHOLD 원천 아님 — appendix_runtime_metadata와 구분할 것.
- 손으로 `condition_mapping_candidate`에 임계를 INSERT하는 방식은 불필요(별표 자산에 이미 존재). cmc는 별도 매핑 자산이며 운영 정본 아님(6/25 감사: GPT Compiler는 cmc 미사용).

### 담당 경계
- 별표 THRESHOLD → obligation_instance 발화 로직 = **Applicability Engine(GPT 소관)**. Claude는 자산 직독·검증·문서.
- 확정 필요: 이 연결이 Applicability 내부 활성화(품질 개발 범위)인지 GPT 확인.
...
2. THRESHOLD는 키워드 매칭 아닌 매핑 테이블로 연결 (→ 위 갱신: 별표 THRESHOLD 24,626 자산 활용)
...
현재 applicability_conditions 14건 (소량 파일럿).
**※ 2026-07-01 갱신: 대량 별표 THRESHOLD 자산은 `appendix_runtime_metadata`(24,626건, structured_json)에 존재 — 6장 THRESHOLD 갱신 참조. Route B의 실제 임계 소스는 이 테이블.**
...
```
THRESHOLD 전체 범위      → 별표 THRESHOLD 24,626 자산 활성화 (6장 갱신 참조; 수집은 이미 완료)
...
- **2026-07-01 (17차 연속):** 6장·7장·10장 THRESHOLD 항목 갱신. 별표는 법제처 API로 이미 수집·구조화 완료(`appendix_runtime_metadata` 31,974건, THRESHOLD 24,626건 structured_json). THRESHOLD 병목은 "수집"이 아니라 "운영 obligation_instance 발화 연결(활성화)". 상세는 6장 "THRESHOLD 갱신" 참조. 관련 메모: `docs/2026-06-30_WO-THRESHOLD-QUALITY-002_사이클메모.md`.

---

*TAI Safe 법령의무도출 시스템 설계 및 구현계획서*  
*2026-06-22 | TAI Engineering | 갱신 2026-07-01*
