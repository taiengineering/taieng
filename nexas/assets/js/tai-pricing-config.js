...
window.TAI_PRICING = (function() {
  ...
  function getSaasPlans(sector) {
    ...
    if (_apiSaas && _apiSaas.length) {
      var f = _apiSaas.filter(function(p) { return (p.sector || p.plan_sector || '').toUpperCase() === sector; });
      if (f.length) return normalizeSaasAPI(f);
    }
    return SAAS[sector] || [];
  }
  ...
  function getDiagPlans(sector) {
    ...
    if (_apiDiag && _apiDiag.length) {
      var f = _apiDiag.filter(function(r) { return (r.sector || '').toUpperCase() === sector; });
      if (f.length) return normalizeDiagAPI(f);
    }
    return DIAG[sector] || [];
  }
  ...
  function normalizeSaasAPI(raw) {
    return raw.map(function(p) {
      return { code: p.plan_code||p.code||'', name: p.plan_name||p.display_name||p.name||'', price: parseInt(p.monthly_base_fee||p.price_monthly||p.price||0), basis: p.basis||p.target||'', featured: !!(p.is_recommended||p.featured), custom: !!(p.is_custom||(p.plan_name||'').toUpperCase()==='CUSTOM'), desc: p.description||p.desc||'', features: p.features||[] };
    });
  }
  function normalizeDiagAPI(raw) {
    return raw.map(function(r) {
      return { code: r.plan_code||r.facility_type_code||r.code||'', name: r.plan_name||r.display_name||r.name||'', price: parseInt(r.total_report_fee||r.price||r.amount||0), basis: r.basis||'', featured: !!(r.is_recommended||r.featured), features: r.features||[] };
    });
  }
  ...
  // ══════ Mount: diagnosis.html 가격 카드 렌더링 ══════
    ...
    sectors.forEach(function(s) {
      ...
      html += plans.map(function(p) {
        ...
        return '<div class="' + s.cols + '"><div class="price-card' + (p.featured?' featured':'') + '">'
          ...
          + '<div class="pc-name">' + p.basis + '<br><small style="font-weight:500;color:#94a3b8;font-size:.78rem;">' + fmt(p.price) + '원</small></div>'
  ...
  return {
    ...
    fmt: fmt, annualPrice: annualPrice, annualMonthly: annualMonthly,
